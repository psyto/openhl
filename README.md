...
- **Margin model is mark-aware initial-margin, oracle + maintenance still synthetic.** Stage 17j upgrades the withdraw rule to production shape: when the CLOB has both a bid and an ask, the midpoint is the mark and `free = (collateral + unrealized_pnl) − |size| × mark × im_bps / 10⁴`. Traders can withdraw against unrealized gains; loss positions face a tighter limit than the avg-entry rule. With a one-sided book the Stage 17g avg-entry fallback kicks in (conservative). What's still synthetic: oracle-index mark, separate maintenance-margin checks for solvency margin, and tunable `LiquidationParams` plumbing into the bridge (it still uses the constant `DEFAULT_INITIAL_MARGIN_BPS = 1000`).
- **Revert-aware precompiles are now production-wired.** Stage 17i shipped `OpenHlRevertGuard` (a REVM `Inspector` that snapshots `{accounts, CLOB book, pending_fills}` per call frame and restores on revert). Stage 17k closes the loop: `OpenHlEvmFactory::create_evm` now returns an `OpenHlEvm` wrapper that internally composes the user-facing inspector with the guard via REVM's `Inspector for (L, R)` tuple impl, presenting `Inspector = I` to satisfy the `EvmFactory` trait. The guard runs by default in Reth's BlockExecutor — a contract that calls `openhl_deposit` and then `REVERT`s no longer mints collateral.
- **Follower replication now uses real ProposalAndParts** (Stage 18a). The proposer ships its built block over `NetworkMsg::PublishProposalPart` (Data + Fin), the follower assembles the parts and calls `bridge.register_proposed_block` to install the block in pending without recomputing anything. `Decided` no longer rebuilds. Verified end-to-end on a 2-validator devnet — both nodes commit byte-identical hashes / accounts / coordinator state without `build_payload` ever running on the follower path. The 13n deterministic-recompute shortcut is gone.
