...
- **Solidity-side test is bytecode-only; full-tx path live on the proposer.** Stage 17f deploys a hand-rolled 26-byte wrapper at a contract address in an in-memory revm `CacheDB`, executes a transaction against it via `OpenHlEvmFactory`, and asserts that the EVM `CALL` into `openhl_deposit`/`openhl_withdraw` mutates the bridge's account map. Stage 19d ships a `MarginHealthReader` contract via genesis allocation, reachable via `eth_call`. Stage 20a added `bridge.build_real_payload(parent, attrs)`, invoking Reth's actual `PayloadBuilderService`. Stage 20b wired the real builder into the bridge's `ConsensusBridge::build_payload` flow AND extended `commit` to fire `engine.new_payload(payload)` before `fork_choice_updated`. Stage 20c-1 threads both handles into `bin/openhl/src/main.rs` so the production devnet now produces real Reth-built blocks — `eth_blockNumber` advances in lockstep with consensus (smoke-tested at 30 rounds, restart-resume verified). The `build_real_payload` path bumps `attrs.timestamp` to `parent.timestamp + 1` automatically so the demo's `tick.time = 0` start doesn't trip Reth's strictly-monotonic header check. Remaining for full multi-validator real-payload: extend the Stage 18a `ProposedBlockWire` to ship the `ExecutionPayloadV3` so follower validators also install via their own `engine.new_payload` (today they still only install the Header — their bridge state stays consistent, but their Reth side stays at genesis). That's Stage 20c-2. With 20c-1 in, the next visible step is Stage 20d — `eth_sendRawTransaction` actually landing user transactions in mined blocks.
