...
- **Margin model is production-shape on the bridge / precompile / RPC surface.** Stage 17j upgrades the withdraw rule to `free = (collateral + uPnL) − |size| × mark × im_bps / 10⁴`. 17l → 17m make the full `LiquidationParams` runtime-tunable. 17m exposes `bridge.margin_health(account)`; 17n adds the same classifier as the `openhl_margin_health` precompile at `0x…0c1f`. Stage 17o pipes `openhl-oracle`'s aggregated index price through to the bridge (and precompile global) after every `coordinator.tick`, and switches withdraw + margin_health to consult it as the canonical mark — falling back to the CLOB midpoint only when no oracle refresh has succeeded yet. What's still synthetic: the integration coordinator's per-block liquidation scan still passes the CLOB midpoint as `TickInput.mark` (a separate refactor — the scan reads its own mark independently of the bridge's withdraw / margin_health rule).
